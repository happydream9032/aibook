...
import AIPrompt from "./alprompt";
function TypingComponent() {
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);  //show dropmenu
  const [isComponentType, setComponentType] = useState(0); // choose type of component (ex: importfile, fetchurl)
  ...
  const handleDatafromChildren = (childData) => {
    setComponentType(childData);
  };

  function renderContent() {
    if (isComponentType == 0) {
      return (
    ...
    } else if (isComponentType == 1) {
      return (<div contentEditable={false}><Importfile onData={handleDatafromChildren}/></div>);
    } else if (isComponentType == 2) {
      return (<div contentEditable={false}><RunSQL onData={handleDatafromChildren}/></div>);
    } else if (isComponentType == 4) {
      return (<div contentEditable={false}><AIPrompt onData={handleDatafromChildren}/></div>);
    }
  }
